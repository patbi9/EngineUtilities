...
public:
    ...
    CVector2 operator*(float fac) const {
    ...
    bool operator==(const CVector2& otro) const {
    ...
    float dot(const CVector2& otro) const {
        return x * otro.x + y * otro.y;
    }

    /** @brief Computes the 2D cross product (scalar). */
    float cross(const CVector2& otro) const {
    ...
    void setScale(const CVector2& fac) {
        x = fac.x;
        y = fac.y;
    }

    /** @brief Scales this vector component-wise. */
    void scale(const CVector2& fac) {
        x *= fac.x;
        y *= fac.y;
    }
