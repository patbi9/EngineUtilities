...
namespace EU {
 ...
 class
   ...
   */
  Matrix4x4 
   operator+(const Matrix4x4& otro) const {
   Matrix4x4 r;
   for (int i = 0; i < 4; ++i)
    for (int j = 0; j < 4; ++j)
   ...
   */
  const float& 
   operator()(int fil, int col) const {
   return m[fil][col];
  }
   ...
   */
  void 
   setScale(float scaX, float scaY, float scaZ) {
   for (int i = 0; i < 4; ++i)
    for (int j = 0; j < 4; ++j)
     m[i][j] = 0.0f;
    m[0][0] = scaX;
